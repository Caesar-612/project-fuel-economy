...
#make ggplot to show the one type which has the highest ghg score in barplot and it mean the cleanest
fuel%>%
  filter(year == "2003")%>%
  filter(fuelType == "Regular")%>%
  unite(make, model, col = "cartype", sep =" ")%>%
  #add city08 to compare with ghgscore
  select(cartype, ghgScore, city08,year)%>%
  unique()%>%
  slice_max(city08, n =5)%>%
...
fuel%>%
  ...
  filter(fuelType == "Regular")%>%
  unite(make, model, col = "cartype", sep =" ")%>%
#  filter(cartype == "Honda Insight" )%>%
  select(cartype, ghgScore, city08,year,trany)%>%
  unique()
fuel%>%
  select(trany)%>%
  unique()
...
```{r}
fuel%>%
  select(year, ghgScore, make, model, city08)%>%
  filter(year == '2020')%>%
   unite(make, model, col = "cartype", sep =" ")%>%
  filter()
...
```{r}
#fuelCost08 - annual fuel cost, displ - engine displacement in liters
fuel%>%
  select(displ, fuelCost08)%>%
  ggplot(aes( x= displ, y= fuelCost08))+
  geom_point()+
  geom_smooth(method = lm, se = FALSE)
  theme_bw()
lm(fuel$fuelCost08~fuel$displ) ->x
summary(x)
#Since the p-value is less than .05 there is an significance meaning between fuelCost08 and displ.
```
...
fuel%>%
  ...
  unite(make, model, col = "cartype", sep =" ")%>%
  unique()%>%
  pivot_wider(names_from = cartype, values_from = city08)
```

```{r}
t.test(fuel$city08,fuel$ghgScore)
```
```{r}
fuel%>%
        filter(year == 2003)%>%
        ...
        unite(make, model, col = "cartype", sep =" ")%>%
  unite(cartype, trany, col = "cartype", sep= " - ")%>%
        unique()%>%
        slice_max(city08,n =5)%>%
        ggplot(aes(x = cartype, y = city08,fill = ))+
        geom_col()+
...
```{r}
#city08 - city MPG,co2 - tailpipe CO2 in grams/mile ,displ - engine displacement in liters, cylinders - engine cylinders, fuelCost08-annual fuel cost, ghgScore - EPA GHG score
#x axis = displ, cylinders y = ghgScore, annual fuel cost, city08, co2
fuel%>%
  keep(is.numeric)-> fuel_2

y = 1.03987
summary(x)$residual%>%summary()
summary(x)$coefficients
fuel_2%>%
  select(displ)
lm(cylinders~fuelCost08, fuel_2)-> x
x
predict.lm(x,data.frame(fuelCost08 = 15))
```
```{r}
data.frame(c(10))-> df
colnames(df)<- "city08"
